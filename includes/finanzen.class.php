...
class finanzenNEW extends functions {
	...
	function showErrors() {
		...
		if ($kontostand [0]->summe > 0 or $kontostand [0]->summe < 0) {
			...
			echo "Achtung, es wurde ein Fehler in mindestens einer Buchung entdeckt.
				...
				dies kann verschiedene Gr�nde haben. Der Administrator wurde �ber
				dieses Problem informiert. Die betroffene Buchung wird jetzt gel�scht!";
			...
			for($i = 0; $i <= $max; $i ++) {
				...
				if ($anzahl != 2 and $anzahl != 0) {
					echo "<p class='meldung'>Unvollst�ndige Buchung: " .$infos[0]->umsatzName. ", ".$infos[0]->umsatzWert."</p>";
					$delete = "DELETE FROM finanzen_umsaetze
					WHERE buchungsnr = '$i' LIMIT 1";
					if ($this->sql_insert_update_delete ( $delete ) == true) {
						$this->logEintrag ( true, "Buchung $i wurde wegen Unvollständigkeit gel�scht.", "Error" );
						echo "<p class='erfolg'>Fehlerhafte Buchung gel�scht</p>";
					}
				}
	...
	/**
	 * Gibt die n�chste freie Buchungsnummer wieder, verwendet auch alte, freie Nummern, welche kleiner als MAX sind.
	 * @return nextnumber
	 */
	...
	/**
	 * Gibt immer die n�chste MAX nummer zur�ck.
    ...
    function createUeberweisung($besitzer, $von, $nach, $text, $betrag, $datum) {
		...
		if ($this->sql_insert_update_delete ( $query ) == true and $this->sql_insert_update_delete ( $query2 ) == true) {
            echo "<p class='erfolg'>�berweisung von $text durchgef�hrt (Buchungsnummer: $buchungsnummer)</p>";
		}
    }
		...
		if (isset ( $_GET ['newUeberweisung'] )) {
			if ($this->userHasRight (18, 0) == true) {
				...
				if (isset ( $_POST ['sendnewUeberweisung'] ) and isset ( $_POST ['valueUeberweisung'] ) and isset ( $_POST ['textUeberweisung'] ) and isset ( $_POST ['dateUeberweisung'] ) and isset ( $_POST ['zielKonto'] ) and isset ( $_POST ['absenderKonto'] )) {
					...
					if ($von == $nach) {
						echo "<p class='meldung'>Absende und Zielkonto ist gleich. Keine Buchung m�glich.</p>";
						exit ();
					}
                    
                    if($datum < $maxpast) {
                        echo "<p class='meldung'>Keine Buchung vor $maxpast m�glich!</p>";
                        exit();
                    }
					...
					if ($von != "" and $nach != "" and isset ( $besitzer ) and $betrag != "" and $betrag > 0 and $datum != "" and $text != "") {
                            ...
                            if(isset($_POST['monthweitere'])) {
                                if(is_numeric($_POST['monthweitere']) == true) {
                                    ...
                                    for ($x = 1 ; $x <= $number ; $x++) {
                                        ...
                                        echo "<p class='dezentInfo'>${x}: Buchung f�r Monat " . $newdate->format("Y-m-d") . "</p>";
						...
						if (isset ( $_POST ['weitere'] )) {
							...
							for($j = 0; $j < $anzahlWeitereFelder; $j ++) {
								...
								// Nur gef�llte Inputfelder verwenden.
								if ($dates [$j] != "") {
                ...
                for($i = 0; $i < sizeof ( $kontenDesNutzers ); $i ++) {
                        ...
                        if (isset ( $jahresabschlusswert [0]->summe )) {
                            if ($jahresabschlusswert [0]->summe != $tatsaechlicheSumme [0]->summe) {
                                echo "<p class='meldung'>Fehler bei Konto $konto, die tatsächliche Summe ist " . $tatsaechlicheSumme [0]->summe . ", aber der eingetragene ist " . $jahresabschlusswert [0]->summe . "Lösche jetzt die Jahresabschlüsse.";
