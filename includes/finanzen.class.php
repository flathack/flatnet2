...
class finanzenNEW extends functions {
	...
	/**
	 * Stellt die Funktionalit‰ten f¸r das teilen von Konten zur Verf¸gung
	 */
	...
	#	echo "<p class='info'>$kontoid</p>";
		if(isset($kontoid) AND is_numeric($kontoid) == true) {
			...
			# Checken ob ein Share f¸r diesen Benutzer vorhanden ist ...
	...
	function showCurrentMonthInKonto($besitzer) {
		...
		if ($kontoID > 0) {
			...
			if($kontoinfos[0]->art == 2) {
				...
				$saldotext = "/ Startsaldo diesen Monat: <strong>$startsaldo ‚Ç¨";
			}
			echo "<thead>";
	...
	function regelmaessigeZahlung($umsatz, $besitzer) {
		...
		# ƒhnliche Zahlungen vom Nutzer suchen ...
		...
		if ($this->userHasRight (18, 0 ) == true) {
			if (isset ( $_GET ['edit'] )) {
				...
				if(isset($umsatzInfo[0]->id)) {
					...
					echo "<a href='?konto=$kontoLink&monat=$monat&jahr=$jahr' class='rightRedLink'>schlie√üen</a>";
	...
	/**
	 * Neue √úberweisung.
	 * Erm&ouml;glicht das √úberweisen von Geld von einem auf ein anderes Konto.
	 */
	function showCreateNewUeberweisung() {
		if (isset ( $_GET ['newUeberweisung'] )) {
			...
			#	echo "<a href='#' onclick='window.history.go(-1)' target='_self' class='highlightedLink'>Zur&uuml;ck</a>";
				...
				# WEITERE BUCHUNGEN DURCHF√úHREN
				...
				echo "<label for='weitere'>Diese √úberweisung f&uuml;r folgende weitere Tage durchf&uuml;hren</label>";
				echo "</td></tbody>";
				$j = 0;
	...
	function showEditKonto($besitzer) {
		...
		if (isset ( $_GET ['editKonto'] )) {
			...
			echo "<p>√úbersicht vorhandener Buchungen</p>";
			echo "<table class='flatnetTable'>";
	...
	/**
	 * √Ñndert das Konto einer Buchungsnummer
	 * 
		...
		if (isset ( $_POST ['absenden'] )) {
			if (isset ( $_POST ['gutschriftKonto'] ) and isset ( $_POST ['absenderKonto'] )) {
				...
				for($i = 0; $i < sizeof ( $getBuchungInfos ); $i ++) {
					// √Ñnderung f&uuml;r den ABSENDER
					...
					// √ÑNDERUNG f&uuml;r die GUTSCHRIFT
					if ($getBuchungInfos [$i]->umsatzWert > 0) {
	...
	/**
	 * Zeigt eine Auswahl zum √Ñndern deiner Buchungsnummer bez&uuml;gl.
        ...
        if(isset($_GET['konto']) AND isset($_GET['jahr'])) {
            ...
            if(isset($jahresabschlussFuerDiesesJahr[0]->wert) AND $konto > 0) {
                echo "<div class='dezentInfo'>";
                    echo "<p>" ."F&uuml;r dieses Jahr gibt es einen Jahresabschluss, der Endwert betr&auml;gt: " .$jahresabschlussFuerDiesesJahr[0]->wert. " ‚Ç¨". "</p>";
                echo "</div>";
            }
	...
	/**
	 * √úberpr&uuml;ft alle Jahresabschl&uuml;sse auf Richtigkeit und l&ouml;scht diese, fall n&ouml;tig.
