...
class guildwars extends functions {
	...
	function getBesitzer($id) {
		...
		if(!isset($row->besitzer) OR $row->besitzer == "") {
			$name = "Kein Besitzer";
		} else {
			$name = $row->besitzer;
		}
	...
	function selectAccount() {
		...
		$ergebnis2 = mysql_query($getAccounts);

		$menge = $this->getAmount("SELECT * FROM gw_accounts WHERE besitzer = '$user'");
		...
		while($row2 = mysql_fetch_object($ergebnis2)) {
			...
			echo "<input onChange='this.form.submit();' type=radio value='$row2->account' name='gw_account' id='$row2->mail' onclick=''";
			if($_SESSION['gw_account'] == $row2->account){
				echo "checked";
			} else {
				echo "unchecked";
			}
			echo " />";
			echo "<label for='$row2->mail'>$row2->mail</label>";
...
class gw_costs extends guildwars { 
	...
	function showCostCalcEntries() {
		if($this->userHasRight("63", 0) == true) {
			...
			while ($row = mysql_fetch_object($ergebnis)) {
				$summe = $summe + $row->wert;
				echo  "<tbody><td>" . $row->tag . "." . $row->monat . "." . $row->jahr . "</td><td>" . $row->text . "</td><td>$row->wert €</td><td><a href='?loeschen&loeschid=$row->id' class='highlightedLink'>X</a></td></tbody>";
			}
	 ...
	 * @param unknown $text
	 * @param unknown $wert
	 * @param unknown $kaufdat
	 */
	function setGWCostEintrag($text, $wert, $kaufdat) {
		if($this->userHasRight("63", 0) == true) {
	...
	function gwCharKalender() {
		...
		if($this->userHasRight("62", 0) == true) {
			...
			for($i = 1; $i <= 12; $i++ ) {
				...
				$j = 1;
				if($menge > 4) { # wenn nicht genug Platz ist.
					while($row = mysql_fetch_object($ergebnis) AND $j <= 4) {
	
						$monat = $row->monat;
	
						echo "<a href='charakter.php?charID=$row->id'><strong>". $row->tag . ".</strong> "
								. $row->name . " (" . $this->getUserName($row->besitzer) . ") </a><br>";
						$j++;
					...
					while($row = mysql_fetch_object($ergebnis)) {
						...
						echo "<a href='charakter.php?charID=$row->id'><strong>". $row->tag . ".</strong> " . $row->name . " (" . $this->getUserName($row->besitzer) .  ")</a><br>";
			...
			if(isset($_GET['month'])) {
				...
				if($monat != "") {
					...
					while($row = mysql_fetch_object($ergebnis)) {
						...
						$wirdalt = $row->jahr + 1;
						echo "<div class=''>";
						echo "<a href='charakter.php?charID=$row->id'><strong>". $row->tag . "." . $row->monat . ":</strong> " . $row->name . " (" . $this->getUserName($row->besitzer) .  ") wird " . $wirdalt . " Jahre alt</a>";
						echo "</div>";
					}
					echo "</div>";
...
class gw_charakter extends guildwars {
	...
	function selectUser() {
		if($this->userHasRight("3", 0) == true) {
			...
			$menge = $this->getAmount("SELECT Name FROM benutzer ORDER BY Name");
				
			# Benutzer in Array laden
			$allUsers = $this->getObjectsToArray("SELECT id, Name, rights FROM benutzer ORDER BY Name");
			...
			#$getusersErgeb2 = mysql_query($getusers);
			if($menge > $limit) {
	...
	function showChars() {
		if($this->userHasRight("3", 0) == true) {
			...
			# Ausgabe
			while ($dsatz = mysql_fetch_assoc($birthdays)) {
	
				# Erstelldatum in Variable ziehen
				$gebdat = date("$tyear/m/d", strtotime($dsatz["geboren"]));
				...
				$name = $dsatz["name"];
				$klasse = $dsatz["klasse"];
				$spielstunden = $dsatz["spielstunden"];
				$stufe = $dsatz["stufe"];
				$id = $dsatz["id"];
				$rasse = $dsatz["rasse"];
				...
				if($getAllSpielstunden->summe != 0) {
					$ProzenzDerSpielstunden = round($spielstunden / $getAllSpielstunden->summe * 100, 2);
				} else {
				...
				$ausgabe .= "<div class='IconPlacement'>
						<img height=60px width=60px src='../images/Icons/" . $dsatz["klasse"] . "Ico.png'></img></div>";
					
				# Klasse für CSS selektieren
				$ausgabe .= "<div id=";
				if($dsatz["klasse"] == "Krieger") 		{	$ausgabe .= "'gwkrieger'";			}
				if($dsatz["klasse"] == "Wächter") 		{	$ausgabe .= "'gwwaechter'";			}
				if($dsatz["klasse"] == "Dieb")			{ 	$ausgabe .= "'gwdieb'"; 			}
				if($dsatz["klasse"] == "Waldläufer") 	{	$ausgabe .= "'gwwaldlaeufer'";		}
				if($dsatz["klasse"] == "Ingenieur") 	{	$ausgabe .= "'gwengineer'";			}
				if($dsatz["klasse"] == "Elementarmagier") {	$ausgabe .= "'gwelementarmagier'";	}
				if($dsatz["klasse"] == "Nekromant") 	{	$ausgabe .= "'gwnekromant'";		}
				if($dsatz["klasse"] == "Mesmer") 		{	$ausgabe .= "'gwmesmer'";			}
				if($dsatz["klasse"] == "Widergänger") 	{	$ausgabe .= "'gwwidergaenger'";		}
				$ausgabe .= ">";
				...
				$ausgabe .= "
				<div class='IconRasse'>
				<img height=30px width=30px src='../images/Icons/rassen/" . $dsatz["rasse"] . "Ras.png'></img>
				</div>";
	
				if($dsatz["geboren"] != "0000-00-00") {
					$nextBday = $dsatz["jahre"] + 1;
					$ausgabe .= "<p>". $nextBday . " 'er Geburtstag in " . /* $tmp[$days]	.=	*/ $tage . " Tagen </p>";
				}
	
				if($dsatz["jahre"] < 100) {
					$ausgabe .= "<p>Alter: " . $dsatz["jahre"]. " Jahre.</p>";
				}
	
				# Erkundungsbalken anzeigen:
				$erkundungsGroesse = $dsatz["erkundung"] / 100 * 270;
				if($dsatz["erkundung"] == 100) {
					$stern = "&#10039;";
					$ausgabe .= "
					<style>
					.StyleGW" . $dsatz["id"] . " {
				...
				$ausgabe .= "<div id='erkundungAussen'>" . $dsatz["erkundung"] . " % Erkundet <strong>$stern</strong>" . "</div><p class='StyleGW". $dsatz["id"] ."'></p>";
	
				# Bearbeitet am:
				$ausgabe .= "<p id='lowerInfo'>Bearbeitet " . $dsatz["tag"] . "." . $dsatz["monat"] . "." . $dsatz["jahr"] . "</p>";
			...
			$getAnzahlGwChars = $this->getAmount("SELECT id FROM gw_chars WHERE besitzer = '$user' AND account = '$account' ");
			$getAnzahlGwChars = $getAnzahlGwChars / 3;
				
			for ($i = 0; $i < $getAnzahlGwChars ; $i++) {
	...
	function ListOfChars() {
		if($this->userHasRight("61", 0) == true) {
			...
			$ergebnis = mysql_query($getchars);
			$ausgabe .= "<ul>";
			while($row = mysql_fetch_object($ergebnis)) {
				$ausgabe .= "<li><a href='?charID=$row->id'>$row->name</a></li>";
			}
			$ausgabe .= "</ul>";
			...
			if($_GET['createNewChar'] == "yes") {
				...
				if($this->userHasRight("9", 0) == true) {
					...
					while($row2 = mysql_fetch_object($ergebnis2)) {
						...
						$ausgabe.= " value='$row2->account'>" . $row2->mail . "</option>";
					}
					...
					#$ausgabe.= "<tr><td>Notizen</td></tr>";
	
					#$ausgabe.= "<tr>";
					#$ausgabe.= "<td colspan='4'><textarea name='newnotizen' class='ckeditor' id='newNotizen'> </textarea></td>";
					#$ausgabe.= "</tr>";
					$ausgabe.= "<tr></tr>";
		...
		if(isset($_POST['action'])) {
			if($_POST['action'] == "speichern") {
				...
				if($this->userHasRight("9", 0) == true AND $this->getUserID($_SESSION['username']) == $besitzer OR $this->userHasRight("21", 0) == true) {
					...
					$update = mysql_query($charUpdate) OR die("Error: $charUpdate <br>".mysql_error());
	...
	function delChar($id, $tabelle) {
		...
		if($this->userHasRight("9", 0) == true AND $this->getUserID($_SESSION['username']) == $this->getBesitzer($loeschid) OR $this->userHasRight("21", 0) == true) {
			...
			if($jaloeschen == "Ja") {
				...
				$ergebnis = mysql_query($loeschQuery) OR die("Error: $loeschQuery <br>".mysql_error());
	
				if($ergebnis == true) {
					echo "<p class='erfolg'>Der Charakter wurde gelöscht!</p>";
					exit;
				} else {
					echo "<p id='meldung'>Es gab einen Fehler.</p>";
				}
			}
		}
	}
	...
	function saveCharakterStunden($id) {
		...
		$neuerWert = $bisherigeStunden->wert + $charInfos->spielstunden;
	
		# Update oder Insert durchführen.
		if(!isset($bisherigeStunden->wert) OR $bisherigeStunden->wert == "") {
	...
	function globalCharInfos() {
		...
		if($this->userHasRight("64", 0) == true) {
			...
			$eigeneErkundung = $eigeneWeltErkundet->summe / 100;
			$alleErkundung = $alleWeltErkundet->summe / 100;
			...
			$eigeneStundenGesamt = $EigeneGelöschteStunden->summe + $alleEigenenStunden->summe;
			$allePlusGelöschte = $alleSpielstunden->summe + $gelöschteStundenGesamt->summe;
				
			$prozent = round($alleEigenenStunden->summe / $alleSpielstunden->summe * 100, 2);
			...
			davon gelöscht:<br> <strong>" . $EigeneGelöschteStunden->summe  . "</strong> <br>
			...
			echo "<tr><td>Anzahl Chars: " . $summeChars->anzahl . " Stk.</td></tr>";
...
class gw_handwerk extends guildwars {
	...
	function showMatList() {
		...
		#	$gesamtanzahl = 351;
			...
			$neueGesamtZahlMitErweiterung = $max->max;
			...
			for ($i = 0; $i < $anzahlKategorien; $i++) {
				...
				$kat = $i +1;
				$matsDieserKat = $this->getObjectsToArray("SELECT * FROM gwmatlist WHERE kategorie = '$kat' ORDER BY matID");
				$zähler = 0;
				echo "<tbody>";
	...
	function createNewMats() {
		if($this->userHasRight("21", 0) == true) { # Nur wenn er GW Admin ist.
			...
			echo "<p>Es gibt derzeit " . $max->max . " Materialien. </p>";
	 ...
	 * @return number
	 */
	function getMatAnzahlFromCurrentUser($mat) {
		$user = $this->getUserID($_SESSION['username']);
		...
		if(isset($row->matAnzahl)) {
			$anzahl = $row->matAnzahl;
		} else {
			$anzahl = 0;
		}
	...
	function saveAllMats($gesamtzahl) {
		if(isset($_POST['absenden'])) {
			...
			#	$gesamtzahl = 390;
				...
				if($fehler > 0) {
					echo "<p class='meldung'>Fehler: " . $fehler;
	 ...
	 * @return boolean
	 */
	function matIDexistsForUser($id) {
	...
	/**
	 * Zeigt den Knopf zum Namenspeichern
	 */
	function showSaveAllNamesButton() {
		if($this->userHasRight("16", 0) == true) {
			echo "<input type=submit name=saveMatNames value='Namen speichern' />";
		}
	}
	
	/**
	 * Gibt die Einkaufsliste aus.
	 * @param unknown $matids
	 * @param unknown $matAnzahl
	 * @param unknown $matNames
	 */
	function einkaufsliste($matids, $matAnzahl, $matNames) {
	...
	function userHasACharWithHandwerkOverMax($user, $beruf) {
		...
		$summe = $this->getAmount($select1) + $this->getAmount($select2);
	...
	function charInBeruf($beruf) {
		...
		if($this->userHasRight("3", 0) == true) {
			...
			$ergebnis = mysql_query($select);
			while($row = mysql_fetch_object($ergebnis)) {
				if($row->handwerk1 == $beruf) {
					if($row->handwerk1stufe == 500 OR $row->handwerk1stufe >= 400) {
						$class = "bereichLinkMaster"; $mark = "&checkmark;";
					} else { $class = "bereichLink2"; $mark = "";
					}
				} else {
					if($row->handwerk2stufe == 500 OR $row->handwerk2stufe >= 400) {
						$class = "bereichLinkMaster"; $mark = "&checkmark;";
					} else { $class = "bereichLink2"; $mark = "";
					}
				}
					
				$ausgabe .= "<div class='$class'><a href='charakter.php?charID=$row->id' class=''>" . $row->name . " ";
				if($row->handwerk1 == $beruf) {
					$ausgabe .= "(" . $row->handwerk1stufe . ") $mark</a></div>";
				} else {
					$ausgabe .= "(" . $row->handwerk2stufe . ") $mark</a></div>";
				}
			}
	...
	function berufInfoWaidmann() {
		...
		$matids[6] = 3; $matAnzahl[6] = 100; $matNames[6] = "Kupfererz";
		...
		$matids[18] = 3; $matAnzahl[18] = 9; $matNames[18] = "Knochensplitter";
		$matids[19] = 3; $matAnzahl[19] = 56; $matNames[19] = "Winziger Giftbeutel";
		$matids[20] = 3; $matAnzahl[20] = 12; $matNames[20] = "Winzige Klauen";
		$matids[21] = 3; $matAnzahl[21] = 12; $matNames[21] = "Winzige Totem";
		$matids[22] = 3; $matAnzahl[22] = 12; $matNames[22] = "Winzige Schuppe";
		$matids[23] = 3; $matAnzahl[23] = 9; $matNames[23] = "Knochenscherbe";
		$matids[24] = 3; $matAnzahl[24] = 56; $matNames[24] = "Kleiner Giftbeutel";
		$matids[25] = 3; $matAnzahl[25] = 12; $matNames[25] = "Kleiner Fangzahn";
		$matids[26] = 3; $matAnzahl[26] = 12; $matNames[26] = "Kleines Totem";
		$matids[27] = 3; $matAnzahl[27] = 12; $matNames[27] = "Kleine Schuppen";
		$matids[28] = 3; $matAnzahl[28] = 9; $matNames[28] = "Knochen";
		$matids[29] = 3; $matAnzahl[29] = 56; $matNames[29] = "Giftbeutel";
		$matids[30] = 3; $matAnzahl[30] = 12; $matNames[30] = "Fangzahn";
		$matids[31] = 3; $matAnzahl[31] = 12; $matNames[31] = "Totem";
		$matids[32] = 3; $matAnzahl[32] = 12; $matNames[32] = "Schuppen";
		$matids[33] = 3; $matAnzahl[33] = 9; $matNames[33] = "Dicker Knochen";
		$matids[34] = 3; $matAnzahl[34] = 58; $matNames[34] = "Voller Giftbeutel";
		$matids[35] = 3; $matAnzahl[35] = 12; $matNames[35] = "Scharfe Klaue";
		$matids[36] = 3; $matAnzahl[36] = 12; $matNames[36] = "Graviertes Totem";
	...
	function berufInfoWaffenschmied() {
		...
		$matids[5] = 3; $matAnzahl[5] = 372; $matNames[5] = "Alter Holzblock";
		$matids[6] = 3; $matAnzahl[6] = 264; $matNames[6] = "Geschmeidiger Holzblock";
		$matids[7] = 3; $matAnzahl[7] = 576; $matNames[7] = "Eisenerz";
		$matids[8] = 3; $matAnzahl[8] = 192; $matNames[8] = "Platinerz";
		$matids[9] = 3; $matAnzahl[9] = 226; $matNames[9] = "Mithrilerz";
		$matids[10] = 3; $matAnzahl[10] = 264; $matNames[10] = "Abgelagerter Holzblock";
		$matids[11] = 3; $matAnzahl[11] = 198; $matNames[11] = "Harter Holzblock";
		$matids[16] = 3; $matAnzahl[16] = 100; $matNames[16] = "Holzkohlebrocken";
		$matids[17] = 3; $matAnzahl[17] = 96; $matNames[17] = "Primordiumbrocken";
		$matids[18] = 3; $matAnzahl[18] = 12; $matNames[18] = "Knochensplitter";
		$matids[19] = 3; $matAnzahl[19] = 12; $matNames[19] = "Winziger Giftbeutel";
		$matids[20] = 3; $matAnzahl[20] = 12; $matNames[20] = "Winzige Klauen";
		$matids[21] = 3; $matAnzahl[21] = 12; $matNames[21] = "Winzige Totem";
		$matids[22] = 3; $matAnzahl[22] = 12; $matNames[22] = "Winzige Schuppe";
		$matids[24] = 3; $matAnzahl[24] = 62; $matNames[24] = "Kleiner Giftbeutel";
		$matids[25] = 3; $matAnzahl[25] = 12; $matNames[25] = "Kleiner Fangzahn";
		$matids[26] = 3; $matAnzahl[26] = 6; $matNames[26] = "Kleines Totem";
		$matids[27] = 3; $matAnzahl[27] = 12; $matNames[27] = "Kleine Schuppen";
		$matids[29] = 3; $matAnzahl[29] = 56; $matNames[29] = "Giftbeutel";
		$matids[30] = 3; $matAnzahl[30] = 6; $matNames[30] = "Fangzahn";
		$matids[31] = 3; $matAnzahl[31] = 12; $matNames[31] = "Totem";
		$matids[32] = 3; $matAnzahl[32] = 12; $matNames[32] = "Schuppen";
		$matids[15] = 3; $matAnzahl[15] = 56; $matNames[15] = "Klaue";
		$matids[34] = 3; $matAnzahl[34] = 68; $matNames[34] = "Voller Giftbeutel";
		$matids[35] = 3; $matAnzahl[35] = 12; $matNames[35] = "Scharfe Klaue";
		$matids[36] = 3; $matAnzahl[36] = 6; $matNames[36] = "Graviertes Totem";
	...
	function berufInfoRuestungsschmied() {
		...
		$matids[3] = 3; $matAnzahl[3] = 20; $matNames[3] = "Zinnbrocken";
		$matids[4] = 3; $matAnzahl[4] = 18; $matNames[4] = "Knochensplitter";
		$matids[5] = 3; $matAnzahl[5] = 18; $matNames[5] = "Winziger Giftbeutel";
		$matids[6] = 3; $matAnzahl[6] = 18; $matNames[6] = "Phiole mit Schwachem Blut";
		$matids[7] = 3; $matAnzahl[7] = 60; $matNames[7] = "Spule Jutefaden";
		$matids[8] = 3; $matAnzahl[8] = 18; $matNames[8] = "Winzige Schuppe";
		$matids[9] = 3; $matAnzahl[9] = 18; $matNames[9] = "Winzige Klauen";
		$matids[10] = 3; $matAnzahl[10] = 18; $matNames[10] = "Winzige Totem";
		$matids[11] = 3; $matAnzahl[11] = 96; $matNames[11] = "Wollrest";
		$matids[16] = 3; $matAnzahl[16] = 216; $matNames[16] = "Eisenerz";
		$matids[17] = 3; $matAnzahl[17] = 21; $matNames[17] = "Spule Wollfaden";
		$matids[18] = 3; $matAnzahl[18] = 38; $matNames[18] = "Kleiner Giftbeutel";
		$matids[19] = 3; $matAnzahl[19] = 12; $matNames[19] = "Kleiner Fangzahn";
		$matids[20] = 3; $matAnzahl[20] = 12; $matNames[20] = "Kleine Schuppen";
		$matids[21] = 3; $matAnzahl[21] = 12; $matNames[21] = "Kleines Totem";
		$matids[22] = 3; $matAnzahl[22] = 24; $matNames[22] = "Knochenscherbe";
		$matids[24] = 3; $matAnzahl[24] = 96; $matNames[24] = "Baumwollrest";
		$matids[26] = 3; $matAnzahl[26] = 21; $matNames[26] = "Spule Baumwollfaden";
		$matids[27] = 3; $matAnzahl[27] = 38; $matNames[27] = "Giftbeutel";
		$matids[29] = 3; $matAnzahl[29] = 12; $matNames[29] = "Fangzahn";
		$matids[30] = 3; $matAnzahl[30] = 12; $matNames[30] = "Totem";
		$matids[31] = 3; $matAnzahl[31] = 12; $matNames[31] = "Schuppen";
		$matids[32] = 3; $matAnzahl[32] = 24; $matNames[32] = "Knochen";
		$matids[15] = 3; $matAnzahl[15] = 96; $matNames[15] = "Leinenrest";
		$matids[34] = 3; $matAnzahl[34] = 108; $matNames[34] = "Platinerz";
		$matids[35] = 3; $matAnzahl[35] = 21; $matNames[35] = "Spule Leinenfaden";
		$matids[36] = 3; $matAnzahl[36] = 6; $matNames[36] = "Scharfer Fangzahn";
		$matids[37] = 3; $matAnzahl[37] = 12; $matNames[37] = "Scharfe Klaue";
		$matids[38] = 3; $matAnzahl[38] = 12; $matNames[38] = "Graviertes Totem";
		$matids[39] = 3; $matAnzahl[39] = 44; $matNames[39] = "Voller Giftbeutel";
		$matids[40] = 3; $matAnzahl[40] = 24; $matNames[40] = "Dicker Knochen";
	...
	function berufInfoJuwelier() {
		$matids[0] = 0; $matAnzahl[0] = 0; $matNames[0] = "";
		$matids[1] = 1; $matAnzahl[1] = 279; $matNames[1] = "Kupfererz";
		$matids[2] = 2; $matAnzahl[2] = 198; $matNames[2] = "Silbererz";
		$matids[3] = 3; $matAnzahl[3] = 198; $matNames[3] = "Golderz";
		$matids[4] = 3; $matAnzahl[4] = 198; $matNames[4] = "Platinerz";
		...
		$matids[16] = 3; $matAnzahl[16] = 6; $matNames[16] = "Sonnensteinnugget";
		$matids[17] = 3; $matAnzahl[17] = 3; $matNames[17] = "Lapisnugget";
		$matids[18] = 3; $matAnzahl[18] = 3; $matNames[18] = "Peridotnugget";
		$matids[19] = 3; $matAnzahl[19] = 3; $matNames[19] = "Spinellnugget";
		$matids[20] = 3; $matAnzahl[20] = 6; $matNames[20] = "Topasbrocken";
		$matids[21] = 3; $matAnzahl[21] = 3; $matNames[21] = "Lapisbrocken";
		$matids[22] = 3; $matAnzahl[22] = 3; $matNames[22] = "Peridotbrocken";
		$matids[24] = 3; $matAnzahl[24] = 3; $matNames[24] = "Spinellbrocken";
		$matids[26] = 3; $matAnzahl[26] = 6; $matNames[26] = "Smaragd-Scherbe";
		$matids[27] = 3; $matAnzahl[27] = 3; $matNames[27] = "Chrysokoll-Scherben";
		$matids[29] = 3; $matAnzahl[29] = 3; $matNames[29] = "Opal-Scherben";
		$matids[30] = 3; $matAnzahl[30] = 3; $matNames[30] = "Saphir-Scherben";
		$matids[31] = 3; $matAnzahl[31] = 36; $matNames[31] = "Smaragdkristalle";
		$matids[32] = 3; $matAnzahl[32] = 3; $matNames[32] = "Chrysokollkristall";
		$matids[15] = 3; $matAnzahl[15] = 3; $matNames[15] = "Opalkristalle";
		$matids[34] = 3; $matAnzahl[34] = 3; $matNames[34] = "Saphirkristalle";
		$matids[35] = 3; $matAnzahl[35] = 6; $matNames[35] = "Ektoplasmakugel";
	...
	function berufInfoKonstrukteur() {
		...
		$matidslvl500[10] = 0; $matAnzahllvl500[10] = 20; $matNameslvl500[10] = "Phiole mit kraftvollem Blut";
	...
	function berufInfoLederer() {
		...
		$matids[5] = 3; $matAnzahl[5] = 66; $matNames[5] = "Spule Jutefaden";
		$matids[6] = 3; $matAnzahl[6] = 18; $matNames[6] = "Winzige Schuppe";
		$matids[7] = 3; $matAnzahl[7] = 18; $matNames[7] = "Winzige Klauen";
		...
		$matids[31] = 3; $matAnzahl[31] = 24; $matNames[31] = "Giftbeutel";
	...
	function berufInfoSchneider() {
		...
		$matids[7] = 3; $matAnzahl[7] = 18; $matNames[7] = "Winzige Schuppe";
		$matids[8] = 3; $matAnzahl[8] = 18; $matNames[8] = "Winzige Klauen";
		$matids[9] = 3; $matAnzahl[9] = 18; $matNames[9] = "Winzige Totem";
		...
		$matids[29] = 3; $matAnzahl[29] = 12; $matNames[29] = "Totem";
		$matids[30] = 3; $matAnzahl[30] = 12; $matNames[30] = "Fangzahn";
		$matids[31] = 3; $matAnzahl[31] = 14; $matNames[31] = "Phiole mit Blut";
		...
		$matids[39] = 3; $matAnzahl[39] = 12; $matNames[39] = "Scharfe Klaue";
		$matids[40] = 3; $matAnzahl[40] = 12; $matNames[40] = "Phiole mit Dickem Blut";
	...
	function berufInfoKoch() {
		...
		$matids[24] = 3; $matAnzahl[24] = 77; $matNames[24] = "Kirsche";
	 ...
	 */
	function rankingHandwerk() {
		echo "<div class='neuerBlog'>";
		echo "<h2>Handwerks-Ranking</h2>";
	
		$currentUserID = $this->getUserID($_SESSION['username']);
		$UserRanking = $this->getObjectsToArray("SELECT *, sum(matAnzahl) as summe FROM gwusersmats WHERE besitzer = '$currentUserID'");
		$UserRanking2 = $this->getObjectsToArray("SELECT *, sum(matAnzahl) as summe FROM gwusersmats GROUP BY besitzer ORDER BY summe DESC");
	
	
		echo "Du hast insgesamt ";
		echo $UserRanking[0]->summe . " eingelagerte Materialien in deiner Truhe!<br> ";
		...
		for ($i = 0 ; $i < sizeof($UserRanking2) ; $i++) {
			...
			$letzterEintrag = $this->getObjektInfo("SELECT *
			...
			echo $letzterEintrag->tag . ".". $letzterEintrag->monat .".". $letzterEintrag->jahr ." / ". $letzterEintrag->stunde .":". $letzterEintrag->minute;
			echo "</li>";
			$counter += 1;
		}
		echo "</ul>";
...
class gw_animals extends guildwars {
	
	/**
	 * Zeigt eine Liste der Tiere an.
	 */
	function showAnimals() {
		...
		$selectAnimals = "SELECT id, tierName, tierStandort, nameWegmarke FROM gw_animals ORDER BY tierStandort DESC";
		$ergebnis = mysql_query($selectAnimals);
	
		$ausgabe .= "<table class='flatnetTable'>";
		$ausgabe .= "<thead><td>Name</td><td>Gebiet</td><td>Wegmarke oder Beschreibung</td><td>Options</td></thead>";
		while($row = mysql_fetch_object($ergebnis)) {
			$ausgabe .= "<tbody>
			<td><a href='https://www.google.com/webhp?tab=ww&ei=bi3TVJSGIpDSaK--gsAL&ved=0CAkQ1S4#q=Guild+Wars+2+$row->tierName'>" . substr($row->tierName, 0, 21) . "</a></td>
			<td><a href='https://www.google.com/webhp?tab=ww&ei=bi3TVJSGIpDSaK--gsAL&ved=0CAkQ1S4#q=Guild+Wars+2+$row->tierStandort'>$row->tierStandort</a></td>
			<td><a href='https://www.google.com/webhp?tab=ww&ei=bi3TVJSGIpDSaK--gsAL&ved=0CAkQ1S4#q=Guild+Wars+2+$row->nameWegmarke'>$row->nameWegmarke</a></td>
			<td>
			<a href='?bearb=$row->id' class='buttonlink'>edit</a>
			<a href='?loesch=$row->id' class='highlightedLink'>X</a>
			</td>
			</tbody>";
		}
		echo "</table>";
	...
	function newAnimal() {
		...
		if(isset($_GET['newAni'])) {
			if($_GET['newAni'] == "yes") {
				...
				if($this->userHasRight("9", 0) == true) {
					$ausgabe .= "<form method=post>";
					$ausgabe .= "<br><input type='text' name='aniName' value ='' placeholder='Tiername' />";
					$ausgabe .= "<input type='text' name='aniStandort' value ='' placeholder='Standort' />";
					$ausgabe .= "<input type='text' name='aniWegmarke' value ='' placeholder='Wegmarke' />";
					$ausgabe .= "<input type='submit' name='absenden' value ='anlegen' />";
					$ausgabe .= "</form>";
				} else {
					$meldung .= "<p class='meldung'>Keine Berechtigung</p>";
				}
			}
		}
		/*
		 * Speicherung der Daten in DB
			*/
		if(isset($_POST['absenden'])) {
			if($_POST['absenden'] == "anlegen") {
				# SQL
				if($_POST['aniName'] == "") {
					$meldung .= "<p class='meldung'>Es muss mindestens ein Name eingetragen werden.</p>";
				} else {
					# Variablen zuweisen:
					$aniName = $_POST['aniName'];
					# check ob name bereits existiert.
					$check = "SELECT tierName FROM gw_animals WHERE tierName = '$aniName'";
					$checkergebnis = mysql_query($check);
					$row = mysql_fetch_object($checkergebnis);
					if(isset($row->tierName)) {
						$meldung .= "<p class='meldung'>Das Tier ist in der Datenbank bereits vorhanden</p>";
					} else {
	
						# Rest der Variablen zuweisen
						$aniName = $_POST['aniName'];
						$aniStandort = $_POST['aniStandort'];
						$aniWegmarke = $_POST['aniWegmarke'];
	
	
						$insert="INSERT INTO gw_animals (tierName, tierStandort, nameWegmarke) VALUES ('$aniName','$aniStandort','$aniWegmarke')";
						$ergebnis = mysql_query($insert);
						if($ergebnis == true) {
							$meldung .= '<p class="erfolg">Tier wurde angelegt.</p>';
						} else {
	...
	/**
	 * Ermöglicht das bearbeiten von den Tieren in Guildwars.
	 */
	function bearbAnimal($id) {
		...
		if($this->userHasRight("9", 0) == true) {
			...
			$select = "SELECT * FROM gw_animals WHERE id = '$id'";
	
			$ergebnis = mysql_query($select);
			while($row = mysql_fetch_object($ergebnis)) {
				$ausgabe .= "<form action='?saveBearbAni=1' method=post>";
				$ausgabe .= "<input type='hidden' name='tierID' value='$row->id' />";
				$ausgabe .= "<input type='text' name='tierName' value='$row->tierName' />";
				$ausgabe .= "<input type='text' name='tierStandort' value='$row->tierStandort' />";
				$ausgabe .= "<input type='text' name='nameWegmarke' value='$row->nameWegmarke' />";
				$ausgabe .= "<input type='submit' name='submit' value='speichern' /><br><br>";
				$ausgabe .= "</form>";
			}
	
			if(isset($_GET['saveBearbAni'])) {
				$newAniName = $_POST['tierName'];
				$newAniStandort = $_POST['tierStandort'];
				$newAniWegmarke = $_POST['nameWegmarke'];
				$aniID = $_POST['tierID'];
	
				$animalUpdate = "UPDATE gw_animals SET
				tierName='$newAniName', tierStandort='$newAniStandort', nameWegmarke='$newAniWegmarke'
				WHERE id='$aniID'";
	
				$update = mysql_query($animalUpdate);
	
				if($update == true) {
					$meldung .= "<p class='erfolg'>Änderungen gespeichert.</p><br>";
				}
			}
		}
	...
	/**
	 * Löscht einen Eintrag aus Animals
	 */
	function delAnimal($id) {
		...
		if($this->userHasRight("9", 0) == true) {
			if($id != "") {
				$selectID = "SELECT * FROM gw_animals WHERE id = '$id'";
				$ergebnis = mysql_query($selectID);
				$row = mysql_fetch_object($ergebnis);
	
				if(!isset($_POST['loeschen'])) {
					$ausgabe .= "<p class='meldung'>Sicher, dass <strong>$row->tierName</strong> gelöscht werden soll?" . "<br><br>";
					$ausgabe .= "<form action='?' method=post>";
					$ausgabe .= "<input type='hidden' name='loeschID' value='$row->id' />";
					$ausgabe .= "<input type='submit' name='loeschen' value='Ja' /></p>";
					$ausgabe .= "</form>";
				}
			}
			if(isset($_POST['loeschen'])) {
				if($_POST['loeschen'] == "Ja") {
					...
					$loeschQuery = "DELETE FROM `gw_animals` WHERE `id` = $loeschID";
					$ergebnis = mysql_query($loeschQuery);
					if($ergebnis == true) {
						$meldung .= "<p class='erfolg'>Tier gelöscht <a href='?' class='highlightedlink'>OK</a></p><br><br>";
					} else {
						$meldung .= "<p id='meldung'>Fehler</p><br><br>";
					}
				}
			}
		}
